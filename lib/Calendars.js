...
exports.default = exports.CDate = exports.BaseCalendar = void 0;
...
class CDate {
  ...
  constructor(calendar, yearOrDate, month, day) {
    ...
    if (yearOrDate instanceof CDate) {
      if (this.cal.name !== yearOrDate.calendar().name) {
        throw Calendars.local.differentCalendars.replace(/\{0\}/, this.cal.local.name).replace(/\{1\}/, yearOrDate.calendar().local.name);
      }
  ...
  compareTo(date) {
    if (this.cal.name !== date.cal.name) {
      throw Calendars.local.differentCalendars.replace(/\{0\}/, this.cal.local.name).replace(/\{1\}/, date.cal.local.name);
    }
...
class BaseCalendar {
  ...
  constructor(name, jdEpoch, local, daysPerMonth) {
    var monthsPerYear = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 12;
    var hasYearZero = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
    var minMonth = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : 1;
    var firstMonth = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : 1;
    var minDay = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : 1;
  ...
  validate(error, yearOrDate, month, day, validOptions) {
    if (yearOrDate instanceof CDate) {
      if (this.name !== yearOrDate.calendar().name) {
        throw Calendars.local.differentCalendars.replace(/\{0\}/, this.local.name).replace(/\{1\}/, yearOrDate.calendar().local.name);
      }
...
class Calendars {
  ...
  static date(yearOrDate, month, day, calendar, language) {
    ...
    var cal = calendar instanceof BaseCalendar ? calendar : this.instance(calendar, language);
  ...
  static substituteDigits(digits) {
    return value => "".concat(value).replace(/[0-9]/g, digit => digits[Number(digit)]);
  } // Digit substitution function for localising Chinese style numbers via the Calendar digits option.


  static substituteChineseDigits(digits, powers) {
    return value => {
      var localNumber = '';
      var power = 0;
