...
class NepaliCalendar extends _Calendars.CalendarBase {
  ...
  daysInYear(yearOrDate) {
    ...
    for (var monthNum = this.minMonth; monthNum <= 12; monthNum++) {
      days += NEPALI_CALENDAR_DATA[y][monthNum];
    }
    return days;
  }
  ...
  daysInMonth(yearOrDate, month) {
    ...
    return NEPALI_CALENDAR_DATA[y][m];
  }
  ...
  fromJD(jd) {
    ...
    if (nepaliYear < MIN_YEAR - 1 || nepaliYear > MAX_YEAR) {
      throw new _Calendars.CalendarError(_Calendars.default.local.invalidYear.replace(/\{0\}/, this.local.name));
    }
