...
class NepaliCalendar extends _Calendars.CalendarBase {
  ...
  constructor() {
    ...
    super('Nepali', 1700709.5, NepaliCalendar.localisations[language] || NepaliCalendar.localisations[''], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30]);
  ...
  daysInYear(yearOrDate) {
    ...
    for (var monthNum = this.minMonth; monthNum <= 12; monthNum++) {
  ...
  daysInMonth(yearOrDate, month) {
    ...
    return typeof NEPALI_CALENDAR_DATA[y] === 'undefined' ? this.daysPerMonth[m - 1] : NEPALI_CALENDAR_DATA[y][m];
  ...
  toJD(yearOrDate, month, day) {
    ...
    var nepaliMonth = m;
    this.createMissingCalendarData(y); // We will add all the days that went by since the 1st January and then we can get the Gregorian Date

    var gregorianDayOfYear = 0; // Get the correct year

    var gregorianYear = y - (m > 9 || m === 9 && d >= NEPALI_CALENDAR_DATA[y][0] ? 56 : 57); // First we add the amount of days in the actual Nepali month as the day of year in the
    ...
    if (nepaliMonth === 9) {
      ...
      gregorianDayOfYear += d - NEPALI_CALENDAR_DATA[y][0]; // For the first days of Paush we are now in negative values,
  ...
  fromJD(jd) {
    var gregorianDate = NepaliCalendar.gregorian.fromJD(jd);
    ...
    var dayOfFirstJanInPaush = NEPALI_CALENDAR_DATA[nepaliYear][0]; // Check how many days are left of Paush.
    ...
    var daysSinceJanFirstToEndOfNepaliMonth = NEPALI_CALENDAR_DATA[nepaliYear][nepaliMonth] - dayOfFirstJanInPaush + 1; // If the Gregorian day-of-year is smaller than or equal to the sum of days between the 1st January and
    ...
    var nepaliDayOfMonth = NEPALI_CALENDAR_DATA[nepaliYear][nepaliMonth] - (daysSinceJanFirstToEndOfNepaliMonth - gregorianDayOfYear);
  ...
  createMissingCalendarData(y) {
    var newData = this.daysPerMonth.slice(0);
    newData.unshift(17);

    for (var newYear = y; newYear < y + 3; newYear++) {
...
_defineProperty(NepaliCalendar, "localisations", {
  '': defaultLocalisation
});

_defineProperty(NepaliCalendar, "gregorian", _Calendars.default.instance('gregorian'));

_defineProperty(NepaliCalendar, "defaultDaysPerYear", 365);

_Calendars.default.register('nepali', NepaliCalendar);
//# sourceMappingURL=Nepali.js.map